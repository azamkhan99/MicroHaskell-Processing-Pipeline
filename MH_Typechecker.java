	...
	throws TypeError, UnknownVariable {
         if (exp.isVAR()) {
             return (env.typeOf(exp.value())) ;
                 ...
                 if (!(c >= '0'|| c <='9')) {
                    throw new TypeError ("Only digit characters are allowed " + c + " which appears in " + exp.value() + " is a non-digit character") ;}
            }       return IntegerType;
          }
                 ...
                 if (exp.value() == "True" || exp.value() == "False") {
                   return BoolType ;}
                 else throw new TypeError (exp.value() + " is not one of \"True\" or \"False\".") ;
          }
         else if (exp.isAPP()) {
               ...
               MH_TYPE e2 = computeType(exp.second(), env) ;
                 ...
                 if (!(e1.left().equals(e2))) {
                    throw new TypeError ("The type of e2: " + e2 + " does not match the argument type expected by e1: " + e1.left()) ;}
                    return e1.right();
          }
               ...
               MH_TYPE second = computeType(exp.second(), env) ;
                 ...
                 if (!(first.equals(IntegerType) && second.equals(IntegerType))) {
                    throw new TypeError (exp.first().value() + " and " + exp.second().value() + " must both be integers")  ;}
                 ...
                 if (exp.infixOp().equals("+") || exp.infixOp().equals("-")) {
                    ...
                    throw new TypeError ( exp.infixOp() + " isn't valid as the Infix operator is not in the language") ;}
          }
               ...
               MH_TYPE third = computeType(exp.third(), env) ;
                 ...
                 if (!first.equals(BoolType)) {
                    throw new TypeError (exp.first().value() + " not a boolean condition") ;}
                 if (!second.equals(third)) {
                    throw new TypeError ("The Argument Types of \"then\" and \"else\" in this if-then-else expression aren't the same") ;}
                    return third ;
                }
